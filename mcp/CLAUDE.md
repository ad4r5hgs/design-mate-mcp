# mcp/CLAUDE.md — MCP Server

The `mcp/` package is a Node.js MCP server (stdio transport). It receives tool calls from AI
clients, validates them with Zod, and relays commands to the canvas over WebSocket. It never
touches the DOM and has no React dependency.
