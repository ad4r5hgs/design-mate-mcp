# canvas/CLAUDE.md — Canvas App

The `canvas/` package is a Vite + React application that hosts a tldraw canvas, a CSS flexbox
layout engine, and a WebSocket relay server. It receives commands from the MCP server, computes
layouts, and renders shapes. It also runs `ws-server.ts` as a standalone Node process alongside
the Vite dev server.

**Coding rules:** Inherited from [`../CLAUDE.md`](../CLAUDE.md) — read it first.
...
## Inherited Rules (summary — full text in `../CLAUDE.md`)
